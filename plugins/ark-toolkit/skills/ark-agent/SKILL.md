---
name: ark-agent
description: 60 交易日的 AI vs 人類實驗之 Agent 軌：盤前產生決策包、Agent 依 ARK 紀律做出買賣建議並鎖定、成交對回、滾動評估。當使用者說「跑今天的決策」「產生決策包」「結算」「看實驗成績」時使用。僅限 macOS。
---

# ark-agent

支撐一個實驗：**AI Agent 使用方舟運算(ARK)系統做台股買賣判斷，與人類使用同系統的判斷，誰準確率高。**
本 skill 只負責 Agent 軌——人類對照軌由使用者另行安排，與這裡無關。
...
> **協定變更（2026-08-10）**：原為「盤前 09:00 前鎖定、使用者手動執行」。改為
> 10:00 盤中的理由是官方教學明載「指標約每小時重算、**台股盤中算出調台股**」，
> 盤前讀到的是前一日盤後的舊值；10:00 也避開 09:00–09:30 的開盤波動。
> 改為自動執行後新增**衛星軌**（見下），evaluate 只統計主軌以維持實驗效度。
...
違反邊界做出的決策在實驗上無效。搜尋過的新聞要在 decision JSON 的
...
| 軌道 | ARK 紀律 | 停損 | 說明 |
|---|---|---|---|
| `core` | 完全適用 | 無 | 只做 ETF。實驗的對照組，evaluate 只統計這一軌 |
...
n≈60 個決策日、前瞻視窗互相重疊（自相關），**不做統計檢定、不宣稱兩軌優劣的定論**。
evaluate 只輸出點估計＋樣本數＋pending 筆數；60 日報酬要等最後一筆決策後再
60 個交易日才齊——完整評估期約 120 個交易日，`pending` 常態存在。
