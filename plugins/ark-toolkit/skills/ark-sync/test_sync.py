...
class TestEnsureCostBasis(unittest.TestCase):
    """第一次 sync 問一次口徑、存進 config 後永久沿用；排程（--no-prompt）不問。"""
    ...
    def test_已有口徑就不問(self):
        ...
        got = sync.ensure_cost_basis(cfg, prompt=True, ask=self.ask(FEES), save=self.save)
        self.assertEqual(got, cfg)
        self.assertEqual((self.asked, self.saved), (0, []))

    def test_沒有_Shioaji_帳戶不問(self):
        got = sync.ensure_cost_basis(FILE_CFG, prompt=True, ask=self.ask(FEES), save=self.save)
        self.assertEqual(got, FILE_CFG)
        self.assertEqual(self.asked, 0)

    def test_no_prompt_時不問也不存_沿用券商原值(self):
        got = sync.ensure_cost_basis(SHIOAJI_CFG, prompt=False, ask=self.ask(FEES), save=self.save)
        self.assertEqual(got, SHIOAJI_CFG)
        self.assertEqual((self.asked, self.saved), (0, []))

    def test_第一次問到答案就寫進_config(self):
        got = sync.ensure_cost_basis(SHIOAJI_CFG, prompt=True, ask=self.ask(FEES), save=self.save)
    ...
    def test_視窗按取消_這次用原值_不存_下次再問(self):
        got = sync.ensure_cost_basis(SHIOAJI_CFG, prompt=True, ask=self.ask(None), save=self.save)
